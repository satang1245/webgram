...
### 상속
부모 객체의 포로토타입 객체를 상속받아 사용함. 코드를 재사용 함으로 불필요한 소스 및 메모리 소모를 줄이기 위해.  
...
```
위 예제의 경우 문제가 없어 보이지만, 실제 생성된 인스턴스를 확인해보면 중복된 프로퍼티와 메소드가 보인다.  
...
모든 객체는 [[Prototype]] 이라는 내부 슬롯을 가지고 있으며, 이는 생성방식에 따라 프로토타입이 결정된다.
    ...
    - var user1 = new User() // 생성자 함수를 이용한 경우 생성자 함수의 프로토타입 객체와 연결하게 된다.
      <br />함수 또한 Object객체에 포함되기 때문에 Object.prototye 또한 사용가능하다.  
      즉, 생성자 함수를 통하여 객체를 생성한 경우 `생성자함수.protoype` & `Function.protoype` & `Object.prototype`   
      모두 사용이 가능하다.
    - 내부 슬롯에 직접적으로 접근은 불가하다.
--- 

### \_\_proto\_\_ (접근자 프로퍼티)
모든 객체는`__proto__` 접근자 프로퍼티를 통해 `[[Prototype]] 내부 슬롯`에 접근 할수 있다.